...
if ( ! class_exists( 'CB_widget_recent_posts_slider' ) ) {
    class CB_widget_recent_posts_slider extends WP_Widget {
    	...
    	function __construct() {
    		...
    		parent::__construct('cb-recent-posts-slider', 'Ness Latest Slider', $widget_ops);
...
?>
        		...
        		<?php  while ( $cb_qry->have_posts() ) : $cb_qry->the_post();
        		 ...
        		 ?>
                    <li class="cb-article clearfix">
                        ...
                        <div class="cb-mask"><?php cb_thumbnail( $cb_post_id, '550', '430'); ?></div>

                        <div class="cb-meta-data"><h4 class="cb-post-title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h4></div>
    ...
    ?>
    		...
    		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo $cb_title; ?>" /></p>

    		<p><label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php _e( 'Number of posts to show:', 'cubell' ); ?></label>
    		<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="text" value="<?php echo $cb_number; ?>" size="3" /></p>

         	<p><label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php  echo "Category:"; ?></label>

    		 <select id="<?php echo $this->get_field_id( 'category' ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'category' ) ); ?>">
